      ...
      <div className="px-4 md:px-6 h-14 flex items-center justify-between gap-4">
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <span className="text-xs text-text-secondary font-medium" suppressHydrationWarning>
            {currentTime ? format(currentTime, "HH:mm:ss") + " WIB" : "--:--:--"}
          </span>
          {lastUpdated && (
        ...
        <div className="border-t border-bg-border bg-bg-primary py-1.5 overflow-hidden">
          <div className="ticker-wrap">
            <div className="ticker-content">
              {[...topArticles.slice(0, 12), ...topArticles.slice(0, 12)].map((article, idx) => (
                <a
                  key={article.id + "-" + idx}
                  href={article.url}
                  target="_blank"
